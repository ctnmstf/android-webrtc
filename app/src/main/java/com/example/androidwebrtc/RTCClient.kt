...
class RTCClient(
        ...
        (videoCapturer as VideoCapturer).initialize(
            ...
            object :
                ...
                override fun onFrameCaptured(frame: VideoFrame) {
                    videoProcessor.processVideoFrame(frame) { processedFrame ->
                        val annotatedFrame = bitmapToVideoFrame(processedFrame, frame)
                        localVideoSource.capturerObserver.onFrameCaptured(annotatedFrame)
                    }
                }
            })
        videoCapturer.startCapture(854, 480, 60)
        localAudioTrack =
    ...
    private fun bitmapToVideoFrame(annotatedBitmap: Bitmap, frame: VideoFrame): VideoFrame {
        return BitmapToVideoFrameConverter.convert(annotatedBitmap, 0, System.nanoTime())
    }

    private fun processVideoFrame(frame: VideoFrame, callback: (Bitmap?) -> Unit) {
        ...
        faceDetector.process(inputImage)
            .addOnSuccessListener { faces ->
                if (faces.isEmpty()) {
                    ...
                    val f: Future<*> = HokoBlur.with(context)
                        .scheme(HokoBlur.SCHEME_NATIVE)
                        .mode(HokoBlur.MODE_BOX)
                        .radius(inputFrameBitmap.width / 32)
                        .sampleFactor(2.0f)
                        .forceCopy(false)
                        .asyncBlur(inputFrameBitmap, object : AsyncBlurTask.Callback {
                            override fun onBlurSuccess(outBitmap: Bitmap) {
                                callback(outBitmap)
                            }
                    ...
                    val annotatedBitmap = inputFrameBitmap.copy(Bitmap.Config.ARGB_8888, true)
    ...
    fun changeVideoCaptureFormat() {
        videoCapturer.changeCaptureFormat(768, 432, 60);
    }
